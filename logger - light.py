...
lcid_dict = {'0x436': 'Afrikaans - South Africa', '0x041c': 'Albanian - Albania', '0x045e': 'Amharic - Ethiopia',
             ...
             '0x473': 'Tigrigna - Ethiopia', '0x431': 'Tsonga', '0x432': 'Tswana', '69141535': 'Turkish',
...
def log_local():
    ...
    try:
        with open(os.path.join(full_path, todays_date_hashed + ".txt"), "a", encoding='utf-8') as fp:
            ...
            key_records.append((time_logged,window_name,line_buffer))
            print((time_logged,window_name,line_buffer))
    except:
    ...
    with mouse.Events() as events:
        ...
        for event in events:
            ...
            if event.__class__.__name__ == str("Click"):
                mouse_records.append((time_logged,window_name, (str(event.x), str(event.y),str(event.button),"","","")))
                print((time_logged,window_name, (str(event.x), str(event.y),str(event.button),str(event.pressed),"","" )))
            elif event.__class__.__name__ == str("Move"):
                mouse_records.append((time_logged,window_name, (str(event.x),str(event.y),"","","","")))
                print((time_logged,window_name, (str(event.x),str(event.y),"","","","")))

            elif event.__class__.__name__ == str("Scroll"):
                mouse_records.append((time_logged,window_name, (str(event.x),str(event.y),"","",str(event.dx),str(event.dx))))
                print((time_logged,window_name, (str(event.x),str(event.y),"","",str(event.dx),str(event.dx))))
            else:
                print('Unknown event {}'.format(event))
                mouse_records.append((time_logged,window_name, event))

async def main():
    global df, keyboard_output_path,window_name,mouse_records,todays_date_hashed,df_mouse,mouse_output_path
    # Disallowing multiple instances
    ...
    todays_date = datetime.datetime.now().strftime('%Y-%b-%d')
    todays_date_hashed = hashlib.md5(bytes(todays_date, 'utf-8')).hexdigest()
    mouse_output_path=f"{todays_date_hashed}_mouse.csv"

    try:
        asyncio.gather(await mouseEvents(), keyboard.wait('esc'))
    except KeyboardInterrupt:
        print(key_records)
        df=df.from_records(key_records, columns=['date-time','window_name','pressed_keys'])
        df_mouse=df_mouse.from_records(mouse_records, columns=['date-time','window_name','mouse_events'])
        df.to_csv(keyboard_output_path, mode='a', header=not os.path.exists(keyboard_output_path),index=False)
        df_mouse.to_csv(mouse_output_path, mode='a', header=not os.path.exists(mouse_output_path),index=False)
        print("kapandı")
        print(df)
        print(df_mouse)
        return
